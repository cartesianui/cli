...
function validateCommand(command?: string) {
  if (!command) {
    exitWithError(
      "Missing command.",
      ["npx ts-node cui.ts <command> <subcommand?> [--lib=<library>] [--entities=<Entity1,Entity2,...>]"]
    );
  }
}
...
function validateSubCommand(command: string, subcommand: string | undefined, allowedCmds: string[]) {
  if (!subcommand || !allowedCmds.includes(subcommand)) {
    exitWithError(
      ...
      [
        `npx ts-node cui.ts ${command} <subcommand> --lib=<library> --entities=<Entity1,...>`,
...
function validateLibrary(command: string, subcommand?: string, library?: string) {
  if (!library) {
    exitWithError(
      ...
      [`npx ts-node cui.ts ${command} ${subcommand || '<subcommand>'} --lib=<library> --entities=<Entity1,...>`]
    );
  }
}
...
function validateEntities(command: string, subcommand: string | undefined, library: string | undefined, entities: string[], min = 1) {
  if (!entities || entities.length < min) {
    exitWithError(
      ...
      [`npx ts-node cui.ts ${command} ${subcommand || '<subcommand>'} --lib=${library || '<library>'} --entities=<Entity1,...>`]
    );
  }
}
...
async function generateLibrary(libraryName, entities) {
  ...
  const templatePath = path.join(__dirname, 'projects', 'bo-template');
  //const destPath = path.join(__dirname, 'generated-libs', libraryName, 'projects', `bo-${libraryName}`);
  const destPath = path.join(__dirname, 'projects', `bo-${libraryName}`);
  ...
  try {
    ...
    // remove template file before generating index file's
...
async function enhanceModels(libraryName) {
  ...
  const destPath = path.join(__dirname, 'projects', `bo-${libraryName}`);
  const dirPath = path.join(destPath, 'src', 'lib', 'models', 'domain');
 ...
 * 
 */
async function copyEntityToLibrary(templateRoot, destRoot, entityName) {
  
  const kebab = kebabCase(entityName);
  const pascal = pascalCase(entityName);

  walkAndCopy(templateRoot, destRoot);

  logInfo('Updating domain model export file content.');
  const domainModelDir = path.join(destRoot, 'lib', 'models', 'domain');
  await enhanceIndexFile(domainModelDir, 'model');

  logInfo('Updating form model export file content.');
  const searchModelDir = path.join(destRoot, 'lib', 'models', 'forms');
  await enhanceIndexFile(searchModelDir, 'search');
  
  logInfo('Updating shared export file content.');
  const sharedDir = path.join(destRoot, 'lib', 'shared');
  await enhanceIndexFile(sharedDir);

  logInfo('Updating store export file content.');
  const storeDir = path.join(destRoot, 'lib', 'store');
  await enhanceIndexFile(storeDir);
  ...
  function walkAndCopy(srcDir, targetDir) {
    ...
    for (const entry of entries) {
      ...
      let relPath = path.relative(templateRoot, srcPath);
      ...
      let destPath = path.join(destRoot, replacedRelPath);
...
async function addEntity(library, entities) {
  ...
  const templateRoot = path.join(__dirname, 'projects', 'bo-template', 'src');
  //const kebabLibrary = kebabCase(library);

  const destLibraryPath = path.join(__dirname, 'projects', `bo-${library}`);
  const destRoot = path.join(__dirname, 'projects', `bo-${library}`, 'src');
  ...
  for (const entityName of entities) {
    ...
    copyEntityToLibrary(templateRoot, destRoot, entityName);
  }

  logInfo('Updating module file content.');
  const moduleFilePath = path.join(destLibraryPath, 'src', 'lib', `${library}.module.ts`);
  await enhanceModuleFileWithEntities(moduleFilePath, entities);
  
  logInfo('Updating sandbox file content.');
  const sandboxFilePath = path.join(destLibraryPath, 'src', 'lib', `${library}.sandbox.ts`);
  const stubFile = entities.length > 1 ? 'sandbox.multi.stub' : 'sandbox.stub';
  const sandboxStubFilePath = path.join(destLibraryPath, 'stub', stubFile);
  ...
  const routingFilePath = path.join(destLibraryPath, 'src', 'lib', `${library}-routing.module.ts`);
  const routingStubFile = entities.length > 1 ? 'routing.stub' : 'routing.stub';
  const routingStubFilePath = path.join(destLibraryPath, 'stub', routingStubFile);
...
switch (command) {
  ...
  case 'struct':
    ...
    printNestedTree('bo-template', tree);
    break;
